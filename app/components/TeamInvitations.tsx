...
export function TeamInvitations({ teamId }: TeamInvitationsProps) {
  ...
  const handleEmailInvitation = (invitation: TeamInvitation) => {
    ...
    window.location.href = `mailto:${
      invitation.email
    }?subject=Team Invitation&body=You've been invited to join a team. Click the following link to accept: ${encodeURIComponent(
      inviteLink
    )}`;
  };
        ...
        {invitations.length === 0 ? (
          ...
          <div className="flex flex-col gap-3">
            {invitations.map((invitation) => (
              <div
                key={invitation.id}
                className="flex flex-col gap-3 justify-between rounded-lg border p-3"
              >
